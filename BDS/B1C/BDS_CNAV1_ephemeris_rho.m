function [sv, others] = BDS_CNAV1_ephemeris_rho(ephemeris, tr, ts0)
% 给定接收信号的时间(tr)和名义上信号发射的时间(ts0)，计算在接收信号时刻地心固连坐标系下，卫星在发射信号时的坐标和信号路径长度
% 接收信号的时间需要是准确的时间，需要将本地钟的钟差修正，剩余误差是本地钟的抖动
% 名义上信号发射的时间是接收到的伪码理论上的发射时间，由于卫星钟的误差，需要补偿才能得到信号实际的发射时间，误差源是码环的跟踪误差
...
% 2.计算卫星在信号发射时刻的位置
[sv0, E] = BDS_CNAV1_ephemeris_sv_ecef(ephemeris, ts);
