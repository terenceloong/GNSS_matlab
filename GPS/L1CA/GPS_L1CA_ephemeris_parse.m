function [ephemeris, ion] = GPS_L1CA_ephemeris_parse(bits)
...
% 所有字的校验
for k=0:49
    if GPS_L1CA_check(bits(k*30+(1:32)))==0 %只要检测到错误就返回
        return
    end
end
...
for k=0:4
    ...
    % 根据校验规则对子帧中的每个字做电平翻转
    for m=1:10
        subframe(30*m+(-29:0)) = GPS_L1CA_bitsFlip(subframe(30*m+(-29:0)), D30);
        D30 = subframe(30*m);
    end
